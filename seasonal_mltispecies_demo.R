...
for(k in 1:length(scen)){
  ...
  matsS1=array(0,c(3,3,100))
  matsS2=array(0,c(3,3,100))
  matsS3=array(0,c(3,3,100))
...
# base: -1,1
means=rbind(cbind(-1.5,1),cbind(-1,0.5),cbind(-1.5,0.5),cbind(-0.5,0.5),cbind(-1.5,1.5))

clim.change.scen=c("-W","-S","-W-S","+W-S","-W+S")
...
### ADDITIONAL SIMULATIONS FOR BASELINE (cov change + var change)

clim.change.sim.temp=NULL
clim.change.sim.stage.temp=NULL
envir.clim.change.sim.temp=NULL
remove.sim.clim.change.temp=NULL
    ...
    for(cc in 1){
      ...
      for(k in 1:48){
        ...
        for(x in simul){
          ...
          env=mvrnorm(years, mu=c(-1,1), covariance)
          ...
          envir.clim.change.sim.temp=rbind(envir.clim.change.sim.temp,data.frame(envW=env[,1],
                                                                       envS=env[,2],
                                                                       sim=x,
                                                                       scen=k,
                                                                       clim.change="base",
          ...
          if(length(unique(densS1))<2|length(unique(densS2))<2|length(unique(densS3))<2){
            ...
            remove.sim.clim.change.temp=rbind(remove.sim.clim.change.temp,data.frame(sim=x,
                                                                           scen=k,
                                                                           clim.change="base",
                                                                           var.change=var.change[cp],
                                                                           cov.change=cov.change[cpx]))
            ...
            clim.change.sim.temp=rbind(clim.change.sim.temp,data.frame(species=c("Prey","Predator1","Predator2"),
                                                             ext=c(ext1,ext2,ext3),
                                                             Dens.mu=c(mu.densS1,mu.densS2,mu.densS3),
                                                             Dens.cv=c(cv.densS1,cv.densS2,cv.densS3),
                                                             seas=c(seas.S1,seas.S2,seas.S3),
            ...
            clim.change.sim.stage.temp=rbind(clim.change.sim.stage.temp,data.frame(species=rep(c("Prey","Predator1","Predator2"),each=3),
                                                                         Dens.mu=c(mu.densS1_stage,mu.densS2_stage,mu.densS3_stage),
                                                                         Dens.cv=c(cv.densS1_stage,cv.densS2_stage,cv.densS3_stage),
                                                                         seas=c(0,seas.S1_N,seas.S1_R,0,seas.S2_N,seas.S2_R,0,seas.S3_N,seas.S3_R),
                                                                         sim=x,
...
envir.clim.change.sim=rbind(envir.clim.change.sim,envir.clim.change.sim.temp)
clim.change.sim=rbind(clim.change.sim,clim.change.sim.temp)
clim.change.sim.stage=rbind(clim.change.sim.stage,clim.change.sim.stage.temp)
# remove.sim.clim.change=rbind(remove.sim.clim.change,remove.sim.clim.change.temp)
